...
func List(c *gin.Context) {
	...
	log.Println("http_request 查看全部在线用户", appId)
...
func Online(c *gin.Context) {
	...
	log.Println("http_request 查看用户是否在线", userId, appIdStr)
	appId, _ := strconv.ParseInt(appIdStr, 10, 32)
...
func SendMessage(c *gin.Context) {
	...
	log.Println("http_request 给用户发送消息", appIdStr, userId, msgId, message)
	...
	if cache.SeqDuplicates(msgId) {
		log.Println("给用户发送消息 重复提交:", msgId)
		controllers.Response(c, common.OK, "", data)
...
func SendMessageAll(c *gin.Context) {
	...
	log.Println("http_request 给全体用户发送消息", appIdStr, userId, msgId, message)
